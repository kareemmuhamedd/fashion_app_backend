...
class RemoveItemFromCart(APIView):
    ...
    def delete(self, request):
        ...
        if not cart_items.filter(id=cart_id).exists():
            return Response({'message': 'Cart item dose not exist'}, status=status.HTTP_404_NOT_FOUND)
        cart_items.filter(id=cart_id).delete()
        return Response({'message': 'Item removed from the cart successfully'}, status=status.HTTP_204_NO_CONTENT)
